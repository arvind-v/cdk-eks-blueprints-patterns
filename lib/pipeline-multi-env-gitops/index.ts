...
export default class PipelineMultiEnvGitops {
    ...
    async buildAsync(scope: Construct, id: string, pipelineProps: PipelineMultiEnvGitopsProps, props?: StackProps) {
        ...
        const devArgoAddonConfig = createArgoAddonConfig('dev', 'git@github.com:arvind-v/eks-blueprints-workloads.git');
        const testArgoAddonConfig = createArgoAddonConfig('test', 'git@github.com:arvind-v/eks-blueprints-workloads.git');
        const prodArgoAddonConfig = createArgoAddonConfig('prod', 'git@github.com:arvind-v/eks-blueprints-workloads.git');

        try {

            // const { gitOwner, gitRepositoryName } = await getRepositoryData();
            const gitOwner = 'arvind-v';
        ...
        argoAdditionalProject.push(
            {
                ...
                sourceRepos: [
                    ...
                    `git@github.com:arvind-v/eks-blueprints-workloads.git`,
                ],
            }
        );
    });
